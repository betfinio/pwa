...
import { DrawerDescription, Input, Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@betfinio/components/ui';
...
function SendAction() {
	...
	const { mutate: sendERC20, isPending: isSendingERC20 } = useSendERC20();
	const { mutate: sendNative, isPending: isSendingNative } = useSendNative();
	...
	const handleSend = () => {
		if (!isValid) return;
		if (currency === 'BET') {
			sendERC20({ to: to as Address, amount: BigInt(amount) });
		} else {
			sendNative({ to: to as Address, amount: BigInt(amount) });
		}
	};
					...
					<AnimatePresence mode="popLayout">
						...
						{isValid && (
							...
							>
								...
								<div className="flex flex-col gap-2">
									<Button className="w-full" disabled={!isValid || !amount || isSending} onClick={handleSend}>
